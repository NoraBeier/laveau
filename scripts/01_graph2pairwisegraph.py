...
def compare_neighborhood(G, neighbor_node1, neighbor_node2, found_node, partner):
    ...
    neighbor_node1_neighborhood = get_edge_attributes_for_node(G,neighbor_node1, found_node)  
...
def compare_bonds(G,node_list,found_node,partner):
    ...
    for node1, node2 in node_list:
        if G.edges[found_node,node1].get('bound') == G.edges[partner,node2].get('bound'):
    ...
    for comp in subgraphs:
        ...
        if len(correct_graphs) == 0:
            ...
            with open('01_RCLASS_Error', 'a') as file:
                file.write(rxn + ' Rule with the Subgraph No. ' + str(forrest_count) + ' doesnt work!' + '\n') 
                ...
                for node, data in i.nodes(data=True):
                    if data['atomtype'] == 'r' and  data['atom'] == 'C':
                        ...
                        if len(neighbors) == 3:
                            correct_neighbors = []
                            for n in neighbors:
                                correct_neighbors.append((i.nodes[n]['atom'],i.edges[node,n]['bound']))
